...
class AbstractModuleTest(tf.test.TestCase):
  ...
  def testMultipleGraphs(self):
    ...
    with tf.Graph().as_default() as graph:
      id_mod(tf.random_uniform(dtype=tf.float32, shape=[42]))
  ...
  def testNameScopeRecording(self):
    ...
    # pylint: disable=not-callable
    id_mod(tf.random_uniform(dtype=tf.float32, shape=[22]))
    self.assertIn(id_mod.name_scopes, (("foo",), ("foo_1",)))
    with tf.name_scope("blah"):
      id_mod(tf.random_uniform(dtype=tf.float32, shape=[23]))
    self.assertIn(id_mod.name_scopes,
                  (("foo", "blah/foo"), ("foo_1", "blah/foo")))
    with tf.name_scope("baz"):
      id_mod(tf.random_uniform(dtype=tf.float32, shape=[24]))
    # pylint: enable=not-callable
    self.assertIn(id_mod.name_scopes,
  ...
  def testSubgraphsRecording(self):
    ...
    # pylint: disable=not-callable
    inputs = tf.random_uniform(dtype=tf.float32, shape=[21])
    outputs = id_mod(inputs)
    with tf.name_scope("blah"):
      blah_inputs = tf.random_uniform(dtype=tf.float32, shape=[22])
      blah_outputs = id_mod(blah_inputs)
    with tf.name_scope("baz"):
      baz_inputs = tf.random_uniform(dtype=tf.float32, shape=[23])
  ...
  def testCustomGetter(self):
    ...
    inputs = tf.random_uniform((10, 10), dtype=tf.float32, seed=42)
    ...
    with tf.variable_scope("scope"):
      ...
      module = SimpleModule(custom_getter={"w.*": custom_getter}, name="mod4")
      module(inputs)  # pylint: disable=not-callable
      self.assertEqual(4, connection_count["x"])  # w

      module = SimpleModule(custom_getter={".*": custom_getter}, name="mod5")
      module(inputs)  # pylint: disable=not-callable
      self.assertEqual(6, connection_count["x"])  # w & b

      err = r"More than one custom_getter matched scope/mod6/w \(w\):.*"
      ...
      with self.assertRaisesRegexp(TypeError, err):
        module = SimpleModule(custom_getter=0, name="mod7")
      with self.assertRaisesRegexp(TypeError, err):
        module = SimpleModule(custom_getter={"w": 0}, name="mod8")

  def testCustomGetterNested(self):

    def unset_trainable_getter(getter, name, *args, **kwargs):
    ...
    with tf.variable_scope("scope"):
      ...
      self.assertEqual(4, len(tfe.implicit_gradients(module)(inputs)))

      module = ComplexModule(custom_getter=unset_trainable_getter, name="mod2")
      with self.assertRaisesRegexp(ValueError, "No trainable variables"):
        tfe.implicit_gradients(module)(inputs)

      module = ComplexModule(
          custom_getter={".*/w": unset_trainable_getter}, name="mod3")
      grads_and_vars = tfe.implicit_gradients(module)(inputs)
      trainable_names = [var.name for (_, var) in grads_and_vars]
      # Only biases are trainable, weights were set untrainable.
      ...
      module = ComplexModule(
          custom_getter={".*/b": unset_trainable_getter}, name="mod4")
      grads_and_vars = tfe.implicit_gradients(module)(inputs)
      trainable_names = [var.name for (_, var) in grads_and_vars]
      # Only weights are trainable, biases were set untrainable.
      self.assertEqual(2, len(trainable_names))
      self.assertIn("scope/mod4/linear_1/w:0", trainable_names)
      self.assertIn("scope/mod4/linear_2/w:0", trainable_names)

      module = ComplexModule(
          custom_getter={".*": unset_trainable_getter}, name="mod5")
      # All variables set untrainable.
      with self.assertRaisesRegexp(ValueError, "No trainable variables"):
        tfe.implicit_gradients(module)(inputs)

      module = ComplexModule(
          custom_getter={"w": unset_trainable_getter}, name="mod6")
      # No variables set untrainable.
      self.assertEqual(4, len(tfe.implicit_gradients(module)(inputs)))
...
class ModuleTest(tf.test.TestCase):
  ...
  def testSharing(self):
    ...
    self.assertAllClose(outputs1.numpy(), outputs2.numpy())
